...
1. Real entry interface and exact local input shape.
1. Every condition, parser/policy step, source-visible preventing layer, and observed local result.
1. Affected principal/resource and demonstrated impact.
1. Severity separation: realistic likelihood, demonstrated impact, overall no greater than impact.
1. Remediation strategy and any `code_changes`, including whether the fix enforces the invariant without merely moving trust.

For `needs_validation`, require it to check:

1. The source path is real and supports only the `claimed_root_cause` stated.
1. Every listed blocker is decisive and not already answerable locally.
1. The candidate names a boundary and a possible concrete result rather than a generic concern.
1. At least one validation-plan field is present and exact. `local` uses a bounded fixture; `deployment` asks an owner to observe a configuration, identity, route, policy, or runtime fact. Do not invent a plan for an inapplicable context, and never send audit traffic to a deployment.
1. The fingerprint matches prior/current records for the same root cause.
...
1. One short security posture summary.
1. A confirmed-findings table: severity, title, affected boundary, and one-line observed result.
1. Each confirmed finding: repository source location, lower-trust principal, target-native bounded reproduction, conditions, actual result, impact, priority rationale, and smallest source fix.
1. A separate `NEEDS VALIDATION` table. Give each lead's title, repository trace, exact blocker, bounded local next step, and safe owner-observed deployment check. Do not assign severity or call it a confirmed vulnerability.
1. Separate hardening notes and positive source patterns.
1. Coverage summary from the ledger: covered, candidate, blocked, and deferred counts, plus important exclusions and the final critic result.
