...
1. **Transform** - apply data transformations
1. **Alert** - create alert rules from this panel
1. Below tabs: visualization-specific options and field config
...
1. Add properties to override (unit, color, alias, thresholds, etc.)
...
1. Use recording rules in Prometheus for expensive queries
1. Set longer min intervals for historical dashboards
1. Use `$__interval` variable in queries to align with time resolution
1. Use `$__rate_interval` instead of hardcoded intervals for rate() queries
1. Avoid too many panels on one dashboard (aim for \<20)
