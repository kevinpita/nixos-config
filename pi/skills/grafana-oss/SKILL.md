...
1. Add a provider in `provisioning/dashboards/default.yaml` (see [§ Dashboard provisioning](#dashboard-provisioning) below)
1. Restart Grafana so the provider config is loaded
1. **Verify the dashboard landed**:
   ```bash
...
1. Restart Grafana
1. **Health-check the data source via API**:
   ```bash
...
1. Mint a token via `POST /api/serviceaccounts/{id}/tokens`
